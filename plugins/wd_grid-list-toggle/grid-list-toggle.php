		...
		class WD_List_Grid {
			...
			function setup_gridlist() {
				if ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
					...
					/*add_action ('woocommerce_after_shop_loop_item',array( $this, 'add_short_content') ,8);*/
				}
			}
			...
			function add_short_content(){
				...
				$rs .= wp_trim_words( strip_tags($content), $num_words = 40, $more = null );
				$rs .= '</div>';
			...
			function gridlist_set_default_view() {
				...
				$default = get_option( 'wd_glt_default' );
