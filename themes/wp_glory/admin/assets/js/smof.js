...
jQuery(document).ready(function($){
	...
	jQuery('.wd-img-choose').on('click',function() {
    	var $fold='.f_'+ jQuery(this).data('id');
		var val = jQuery(this).find('input[type=radio]').val();
		var f_val = jQuery($fold).data('fold_val');
		if(typeof f_val !== 'undefined') {
			var open = false;
			jQuery.each(f_val, function(k,v){
				if(val == v) open = true;
			});
			if(open) jQuery($fold).slideDown('normal', "swing");
			else jQuery($fold).slideUp('normal', "swing");
		}
	...
	$('.wp_import_button').live('click',function(e){
		var id = $(this).attr('data-id');
		$('#'+id).click();
	});
	
	$('.wp_import_input').live('change',function(e){
		var files = e.target.files;
		var data = new FormData();
		var $this_ = $(this);
		data.append('action', 'import_color_theme');
		...
		$.ajax({
			...
			beforeSend: function(o){
				$this_.parents('.action_tool').find('button, input').attr('disabled', 'disabled');
			},
			success: function(o){
				if(o['status'] == 'success') {
					$('#section-'+pa_id+' .of-radio-img-selected').click();
				} else {
					alert(o['msg']);
					$this_.val('');
				}
				$this_.parents('.action_tool').find('button, input').removeAttr('disabled');
			}
		});

	});
	
	$('.wp_remove_button').live('click',function(e){
		var img_selected = $('#section-wd_color_scheme .of-radio-img-selected');
		var color_file = img_selected.siblings('.of-radio-img-radio').val();
		var pa_id = $(this).attr('data-pa-id');
		var $this_ = $(this);
		$.ajax({
			url: ajaxurl,
			...
			success: function(o){
				switch(o['status']){
					case 'error': alert(o['msg']); break;
					case 'success':
						$('#section-'+pa_id+' input[value="'+o['msg']+'"]').parent().find('img.of-radio-img-img').click();
				}
				$this_.parents('.action_tool').find('button, input').removeAttr('disabled');
			}
		});
	...
	$('.wp_export_input').live('click',function(e){	
		var pa_id = $(this).attr('data-pa-id');
		var $this_ = $(this);
		$.ajax({
			url: ajaxurl,
	...
	$('.of-radio-img-img').live('click',function(){
		...
		img_selected.parents('.controls').find('.item_description > *').hide();
		img_selected.parents('.controls').find('.item_description .'+ color_file).show();
