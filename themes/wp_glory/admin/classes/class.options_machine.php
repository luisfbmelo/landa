...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'images':
					...
					$i++;
						...
						$output .= '<span class="wd-img-choose" data-id="'.$value['id'].'">';
				...
				case 'theme_colors':
					...
					$i++;
						...
						$output .= '<input type="radio" id="of-radio-img-' . $value['id'] . $i . '" class="checkbox of-radio-img-radio" value="'.$key.'" name="'.$value['id'].'" '.$checked.' />';
						$output .= '<div class="of-radio-img-label">'. $key .'</div>';
						$output .= '<img title="'.$option['name'].'" src="'.$option['img'].'" alt="" class="of-radio-img-img '. $selected .'" onClick="document.getElementById(\'of-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
						$output .= '</span>';
					}
					$output .= '<div class="item_description">';
					foreach ($value['options'] as $key => $option) {
						if($select_value == $key) $output .= '<div class="'.$key.'" style="display: block">';
						else $output .= '<div class="'.$key.'">';
						$output .= $option['desc'];
						$output .= "</div>";
					}
					$output .= "</div>";
					if(isset($value['actions']) && $value['actions'] == 1){
						$output .= '<div class="action_tool">';
						$output .= Options_Machine::optionsframework_import_function($value['id']);
						$output .= "</div>";
					}
				...
				// Uploader 3.5
				case "import_export_color_theme":

					if(!isset($value['fol_id'])) $value['fol_id'] = 'wd_color_scheme';
					...
					$output .= Options_Machine::optionsframework_import_function($value['id'], $value['fol_id']);
	...
	public static function optionsframework_import_function($id){
		$smof_data = of_get_options();
		ob_start();
		?>
