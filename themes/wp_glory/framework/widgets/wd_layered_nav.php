...
class WP_Widget_Wd_Layered_Nav extends WP_Widget {

	var $wd_widget_cssclass;
	var $wd_widget_description;
	var $wd_widget_idbase;
	var $wd_widget_name;
	...
	public function __construct(){
		...
		$this->wd_widget_cssclass 		= 'wd widget_layered_nav';
		$this->wd_widget_description	= __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'wpdance' );
		$this->wd_widget_idbase 		= 'wd_layered_nav';
		$this->wd_widget_name 			= __( 'Wd Layered Nav', 'wpdance' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->wd_widget_cssclass, 'description' => $this->wd_widget_description );

		parent::__construct('wd_layered_nav', $this->wd_widget_name, $widget_ops);
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'wpdance' ), $woocommerce->attribute_label( $taxonomy ) ) .'</option>';
				...
				foreach ( $terms as $term ) {
					...
					echo ' <input class="wd_input_layered" type="checkbox" '. $checked .' value="">';
					echo '</a></li>' ;
				}
				echo "</ul>";	
			} else {
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'attribute' ); ?>"><?php _e( 'Attribute:', 'wpdance' ) ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'display_type' ); ?>"><?php _e( 'Display Type:', 'wpdance' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'display_type' ) ); ?>">
			<option value="list" <?php selected( $instance['display_type'], 'list' ); ?>><?php _e( 'List', 'wpdance' ); ?></option>
			<option value="dropdown" <?php selected( $instance['display_type'], 'dropdown' ); ?>><?php _e( 'Dropdown', 'wpdance' ); ?></option>
			<option value="checkbox" <?php selected( $instance['display_type'], 'checkbox' ); ?>><?php _e( 'CheckBox', 'wpdance' ); ?></option>
		</select></p>
