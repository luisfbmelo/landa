			...
			?>
				<div class="recentcomments_custom <?php if($count==1) echo " first"; if($count==count($comments)) echo " last";?>">
					<div class="detail123">
						...
						?>
						<h3 class="post_title"><a title="<?php echo ($comment->post_title);?>" href="<?php echo get_permalink($comment->comment_post_ID); ?>"><?php echo ($comment->post_title);?></a></h3>
						<div class="comment-meta">
							<span class="recent_comments_date"><?php echo get_the_date();?></span> &#124; 
							<span class="recent_comments_count"><?php 
							$c_count = ($comment_count == 0)? $c_count = 1: $c_count = $comment_count;
							printf(_n( '1 Comment', '%s Comments', absint($c_count), 'wpdance' ), absint($comment_count));?></span>
						</div>
						
						<p class="comment-body">"<?php echo  string_limit_words(get_comment_text(),15); ?>"</p>
						<a class="text_color" href="<?php echo $comment->comment_author_url ;?>"><?php echo $comment->comment_author;?></a>
					</div>
				</div>
				<?php
			}
...
?>
		...
		<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="number" value="<?php echo esc_attr($number); ?>" min="1" max="8" /></p>
<?php
	}
}
