...
if(!class_exists('WP_Widget_Ew_video')){
	class WP_Widget_Ew_video extends WP_Widget{
		public function __construct(){
			...
			parent::__construct("video-widget", "WD - Videos", $widgetOps, $controlOps);
		...
		function widget($args, $instance){
			...
			<?php if ($instance["video" . $i]) { // Do we embed a video from a website?
				...
				?>
				<p class="description"><?php echo $instance["video" . absint($i) . "-desc"] ?></p>
			</div>
			<?php } ?>
			...
			<ul class="items">
				...
				<li>
				  <a class="<?php echo esc_attr($class); ?>" href="#ew-video-cat-<?php echo absint($i); ?>"><?php echo $instance["video" . absint($i) . "-title"]; ?></a>
				</li>
			...
			<p>
				...
				<select id="<?php echo $this->get_field_id("count"); ?>" name="<?php echo $this->get_field_name("count"); ?>" value="<?php echo $instance["count"]; ?>" style="width: 100%;">
					<?php for ($i = 2; $i <= 10; $i++) {
						...
						<option <?php echo $active; ?> value="<?php echo esc_attr($i); ?>"><?php echo esc_html($i); ?></option>
					<?php } ?>
				</select>
