...
if(!function_exists ('ew_pagination')){
	function ew_pagination($num_pages_per_phrase = 3){
		...
		<div class="wp-pagenavi">	
			<span class='curent-total'>Page <?php echo $paged ; ?> of <?php echo $wp_query->max_num_pages ; ?></span>
			<?php
			if($paged > 1){?>
			...
			for($i=0;$start_page+$i<=min(array($wp_query->max_num_pages,$start_page+$max_page-1));$i++){?>
					...
					<span class="pager current<?php if($i == 0) echo ' first-pager';if($start_page+$i == min(array($wp_query->max_num_pages,$start_page+$max_page-1))) echo ' last-pager';?>"><?php echo $start_page+$i;?></span>
					<?php else:?>
					<a class="pager<?php if($i == 0) echo ' first-pager';if($start_page+$i == min(array($wp_query->max_num_pages,$start_page+$max_page-1))) echo ' last-pager';?>" href="<?php echo $pageLink;?><?php echo 'paged=' . ($start_page + $i); ?>"><?php echo $start_page+$i;?></a>
					<?php endif; ?>
					<?php
			}
			...
			<?php if($paged < $wp_query->max_num_pages){?>
					...
					<a class="last" href="<?php echo esc_url($pageLink);?><?php echo 'paged=' . $wp_query->max_num_pages; //next link ?>"><?php _e('Last','wpdance')?></a>
			<?php }?>
