...
if(!function_exists ('wd_get_embbed_vimeo')){
	function wd_get_embbed_vimeo($video_url,$width,$height){
		...
		return '<iframe src="https://player.vimeo.com/video/111617720?title=0&byline=0&portrait=0" width="'.$width.'" height="'.$height.'" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
	}
}
...
if(!function_exists ('wd_get_embbed_video')){
	function wd_get_embbed_video($video_url,$width = 940,$height = 558){
		...
		$embed_shortcode = "[embed width=\"{$width}\" height=\"{$height}\"]{$video_url}[/embed]";
		return $wp_embed->run_shortcode( $embed_shortcode );
	}
}
