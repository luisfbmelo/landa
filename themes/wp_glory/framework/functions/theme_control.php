...
function my_page_template_redirect(){
	...
	$wd_data['wd_layout_header'] =(isset($wd_data['wd_header_styles']) ? $wd_data['wd_header_styles'] : 'wide' ) ;
	$wd_data['wd_layout_main_content'] = (isset($wd_data['wd_maincontent_styles']) ? $wd_data['wd_maincontent_styles'] : 'wide' );
	$wd_data['wd_layout_footer'] = (isset($wd_data['wd_footer_styles']) ? $wd_data['wd_footer_styles'] : 'wide' );
	...
	if($wp_query->is_page()){
		...
		$wd_data['wd_layout_header'] = (strcmp($page_datas['page_layout'],'0')== 0 || strcmp($page_datas['header_layout'],'wide') == 0) ? (isset($wd_data['wd_layout_styles']) ? $wd_data['wd_header_styles'] : 'wide' ) : $page_datas['header_layout'] ;
		$wd_data['wd_layout_main_content'] = (strcmp($page_datas['page_layout'],'0')== 0 || strcmp($page_datas['main_content_layout'],'wide') == 0) ? (isset($wd_data['wd_maincontent_styles']) ? $wd_data['wd_maincontent_styles'] : 'wide' ) : $page_datas['main_content_layout'] ;
		$wd_data['wd_layout_footer'] = (strcmp($page_datas['page_layout'],'0')== 0 || strcmp($page_datas['footer_layout'],'wide') == 0) ? (isset($wd_data['wd_footer_styles']) ? $wd_data['wd_footer_styles'] : 'wide' ) : $page_datas['footer_layout'] ;
		if($wd_data['wd_layout_style'] == 'boxed') {
			$wd_data['wd_layout_header'] = 'boxed';
			$wd_data['wd_layout_main_content'] = 'boxed';
			$wd_data['wd_layout_footer'] = 'boxed';
		}
	...
	if( is_tax( 'product_cat' ) ){
		...
		if( strlen($_term_config) > 0 ){
			...
			if( is_array($_term_config) && count($_term_config) > 0 ){
				...
				$wd_data['wd_prod_cat_custom_content'] = ( isset($_term_config['cat_custom_content']) && strlen($_term_config['cat_custom_content']) > 0 ) ? $_term_config['cat_custom_content'] : '';
			}
	...
	if(isset($wd_data['wd_prod_button_style']) && absint($wd_data['wd_prod_button_style'])) {
		remove_action( 'wd_woocommerce_shop_loop_buttons', 'wd_list_template_loop_add_to_cart', 14 );
	}
...
//					add_action( 'get_header',  'wd_init_sidebar_replacement' );
		...
		if( !$wd_data['wd_prod_sku'] )
			remove_action( 'woocommerce_single_product_summary', 'wd_template_single_sku', 17 );
			
		if( !$wd_data['wd_prod_available'] )
			remove_action( 'woocommerce_single_product_summary', 'wd_template_single_availability', 19 );
