...
if(!function_exists ('wp_comment_form')){
	function wp_comment_form( $args = array(), $post_id = null ) {
		...
		$defaut = array(
			'comment_author'		=>	__('Your Name*','wpdance'),
			'comment_author_email'	=>	__('Your Email*','wpdance')
		);
		...
		$fields =  array(
			'author' => '<span class="label">'.$defaut['comment_author'].'</span><p class="comment-form-author">' . '<input id="author" class="input-text" name="author" type="text" value="' .$comment_author. '" placeholder="'.$defaut['comment_author'].'" size="30"' . $aria_req . ' />' .
						'</p>',
			'email'  => '<span class="label">'.$defaut['comment_author_email'].'</span><p class="comment-form-email"><input id="email" class="input-text" name="email" type="text" value="' . $comment_author_email . '" size="30"' . $aria_req . ' placeholder="'.$defaut['comment_author_email'].'"/>'.
						'</p>',
		);
		...
		$defaults = array(
			...
			'id_form'              => 'commentform',
			'id_submit'            => 'submit',
			'title_reply'          => __( 'Leave a comment','wpdance' ),
		...
		?>
			<?php if ( comments_open() ) : ?>
				...
				<div id="respond">
					<div class="title"><h3><strong><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?> <small><?php cancel_comment_reply_link( $args['cancel_reply_link'] ); ?></small></strong></h3><div class="line line-30"></div></div>
