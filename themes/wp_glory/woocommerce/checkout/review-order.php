	...
	<table class="shop_table cart woocommerce-checkout-review-order-table">
							...
							<tr class="' . esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ) . '">
								<td class="product-name">' .
									'<div class="wd_product_content"><div class="wd_product_item"><a href="'.get_permalink( $cart_item['product_id'] ).'">'
										.$_product->get_image('wd_cart_dropdown').
									'</a>'.
									/*apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s">&times;</a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), __( 'Remove this item', 'wpdance' ) ), $cart_item_key ). */
									...
									'<div class="wd_product_number">'.apply_filters( 'woocommerce_checkout_item_quantity', '<strong class="product-quantity">' . __('Quantity', 'wpdance'). ": " . $cart_item['quantity'] . '</strong>', $cart_item, $cart_item_key ) .'</div>'.
		...
		<tfoot>
			<tr class="cart-subtotal">
				<th><?php _e( 'Cart Subtotal', 'wpdance' ); ?></th>
