...
<head>
    ...
    <title><?php _e( 'Product Comparison', 'wpdance' ) ?></title>
    <link rel="profile" href="http://gmpg.org/xfn/11" />
	<link rel="stylesheet" id="goodly-compare-lato-css" href="http://fonts.googleapis.com/css?family=Lato&amp;ver=4.1" type="text/css" media="all">
...
<h1>
    ...
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php _e( 'Close window [X]', 'wpdance' ) ?></a><?php endif; ?>
</h1>
        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                <td class="<?php echo esc_attr($product_class); ?>">
                    <a href="<?php echo esc_url( add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ); ?>" data-product_id="<?php echo esc_attr($product->id); ?>"><?php _e( 'Remove', 'wpdance' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
